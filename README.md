...
└── saves/              # 角色存檔資料夾 (v2.0 模組化格式)
    ├── [角色名]-[職業]-[日期]/        # 主存檔
    ├── [角色名]-checkpoint-01/       # 手動存檔點 1
    ├── [角色名]-checkpoint-02/       # 手動存檔點 2
    └── [角色名]-auto/                # 自動存檔
```
...
| `存檔` | 任何時候 | 更新當前角色存檔資料夾 |
| `另存新檔` | 任何時候 | 創建新的存檔點 (checkpoint-01, checkpoint-02...) |
| `讀檔` | 任何時候 | 從主存檔資料夾繼續冒險 |
| `讀檔 01` | 任何時候 | 從存檔點 1 繼續 |
| `讀檔 auto` | 任何時候 | 從自動存檔繼續 |
...
每個角色的存檔為獨立資料夾,包含 8 個模組化檔案 + 遊戲數據 JSON:
...
#### saves/ (存檔資料夾)
每個角色的存檔為獨立資料夾,包含:
- `character.md` - 角色屬性、技能、HP、AC 等
- `equipment.md` - 裝備清單與財產
- `npcs.md` - NPC 關係記錄
- `journal.md` - 冒險日誌 (詳細的每日記錄)
- `quests.md` - 任務進度與待辦事項
- `notes.md` - 未解之謎與 GM 備註
- `data/` - 遊戲數據 JSON 檔案
