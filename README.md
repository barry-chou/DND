...
| `開啟選單` | 顯示互動式主選單 |
| `存檔` | 更新當前主存檔 (save.md) |
| `另存新檔` | 創建新的分支存檔 (save-01.md, save-02.md...) |
| `讀檔` | 從主存檔繼續冒險 |
| `讀檔 01` | 從指定編號的存檔點繼續 |
| `讀檔 auto` | 從最新自動存檔點繼續 |
| `上傳` | 將存檔推送到 GitHub (git add + commit + push) |
| `下載` | 從 GitHub 同步最新資料 (git pull) |

### 互動式選單

系統使用 `AskUserQuestion` 提供可點擊的選項,包括:
- **開始新的冒險** - 創建新角色並開始旅程
- **讀取記錄繼續冒險** - 從現有存檔繼續
- **切換角色** - 在不同角色的冒險分支之間切換
- **離開** - 回到正常 Claude 對話模式
