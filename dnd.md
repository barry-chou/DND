...
執行以下命令：
```bash
cd "C:\Users\user\Desktop\dataStructureEx"
git add save.md
git commit -m "存檔更新 - [遊戲內日期] - [簡短摘要]"
```

**Commit message 格式範例:**
- `存檔更新 - 第11天 - 完成格瑞姆第三次訓練`
- `存檔更新 - 第12天 - 交付布洛格訂單，遇見艾莉娜`
- `存檔更新 - 第15天 - 升級至3級，選擇戰技大師`
...
**操作說明:**
- 將本地的 commit(s) 推送到 GitHub 遠端倉庫
- 遠端倉庫: https://github.com/goodday2871/DND
- 使用 SSH 認證 (已設定)

**成功訊息範例:**
```
To github.com:goodday2871/DND.git
   5c3926c..a1b2c3d  main -> main
```

**注意事項:**
- 確保已先執行「存檔」操作（有新的 commit）
- 需要網路連線
- 如遇到錯誤，檢查網路或 SSH 設定
...
1. **遊戲進行中** → 持續冒險、互動、戰鬥
2. **階段結束/玩家想休息** → 玩家說「**存檔**」
3. **GM 執行存檔操作** → 整理劇情 + 更新 save.md + commit
4. **存檔完成** → 告知玩家本次進度已記錄
5. **需要雲端備份時** → 玩家說「**上傳**」
6. **GM 執行上傳** → git push 到 GitHub
...
| 指令 | Git 操作 | 說明 |
|------|---------|------|
| **存檔** | `git add` + `git commit` | 整理劇情並記錄到本地倉庫 |
| **上傳** | `git push` | 推送到 GitHub 雲端備份 |
